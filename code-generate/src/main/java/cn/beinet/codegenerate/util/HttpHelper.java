...
public final class HttpHelper {
    private static final String USER_AGENT = "Beinet Client 1.0";
    private static final Config _defaultConfig;
    private static final HashMap<String, String> _defaultHeader;
    ...
    private static String GetPage(String strUrl, Config config, String originUrl, int deep) {
        ...
        else if (!StringUtils.hasLength(config.getEncoding()))
            config.setEncoding("UTF-8");
        if (!StringUtils.hasLength(config.getUserAgent()))
...
//        config.setProxy("127.0.0.1:8888");
    ...
    private static String ProcessUrl(String strUrl, String param) {
        ...
        strUrl = strUrl.trim();
        if (StringHelper.indexOfIgnoreCase(strUrl, "http://") != 0 &&
                StringHelper.indexOfIgnoreCase(strUrl, "https://") != 0)
    ...
    private static void SetHeaders(HttpURLConnection connection, Config config, String method) {
        ...
        if (StringUtils.hasLength(config.getReferer()))
    ...
    private static String CombineCookie(String url) {
        ...
        synchronized (_cookies) {
            for (Map.Entry<String, HttpCookie> entry : _cookies.entrySet()) {
                ...
                if (!StringUtils.hasLength(cookDomain) || StringHelper.indexOfIgnoreCase(domain, cookDomain) >= 0)
